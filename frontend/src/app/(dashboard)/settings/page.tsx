      ...
      <Card className="rounded-lg border-0 shadow-sm">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-4">
            <Avatar className="h-14 w-14">
              <AvatarFallback className="text-lg bg-primary/10 text-primary font-semibold">{user?.username[0].toUpperCase()}</AvatarFallback>
            </Avatar>
            <div>
              <CardTitle className="text-base">{user?.display_name || user?.username}</CardTitle>
              <p className="text-sm text-muted-foreground">@{user?.username}</p>
              <div className="flex gap-1.5 mt-1.5">
                {user?.roles.map((r) => <Badge key={r.id} variant="secondary" className="text-[10px] h-5 rounded">{r.name}</Badge>)}
        ...
        <CardContent className="pt-4 space-y-4">
          <div className="grid grid-cols-[120px_1fr] gap-y-3 text-sm">
            ...
            <span><Badge variant={user?.is_active ? "default" : "destructive"} className="text-[10px] h-5 rounded">{user?.is_active ? "Active" : "Inactive"}</Badge></span>
          </div>
        </CardContent>
      </Card>
